...
public class AccentRemover {
    ...
    private static char[] REPLACEMENTS = {
        ...
        'U', 'u',};
    private static char[] SPECIAL_CHARACTERS_NONSPACE = {
        '!', '"', '#', '$', '%',
        '*', '+', ',', ':', '<', '=', '>', '?', '@', '[', '\\', ']', '^',
        '`', '|', '~', 'À', 'Á', 'Â', 'Ã', 'È', 'É', 'Ê', 'Ì', 'Í', 'Ò',
        'Ó', 'Ô', 'Õ', 'Ù', 'Ú', 'Ý', 'à', 'á', 'â', 'ã', 'è', 'é', 'ê',
        ...
        'ộ', 'Ớ', 'ớ', 'Ờ', 'ờ', 'Ở', 'ở', 'Ỡ', 'ỡ', 'Ợ', 'ợ', 'Ụ', 'ụ',
        'Ủ', 'ủ', 'Ứ', 'ứ', 'Ừ', 'ừ', 'Ử', 'ử', 'Ữ', 'ữ', 'Ự', 'ự',};
    private static char[] REPLACEMENTS_NONSPACE = {
        '\0', '\0', '\0', '\0', '\0',
        '\0', '_', '\0', '_', '\0', '\0', '\0', '\0', '\0', '\0', '_',
        '\0', '\0', '\0', '\0', '\0', 'A', 'A', 'A', 'A', 'E', 'E', 'E',
        'I', 'I', 'O', 'O', 'O', 'O', 'U', 'U', 'Y', 'a', 'a', 'a', 'a',
    ...
    public static char removeAccentNonSpace(char ch) {
        int index = Arrays.binarySearch(SPECIAL_CHARACTERS_NONSPACE, ch);
        if (index >= 0) {
            ch = REPLACEMENTS_NONSPACE[index];
        }
        return ch;
    }

    public static String removeAccentNonSpace(String s) {
        StringBuilder sb = new StringBuilder(s);
        for (int i = 0; i < sb.length(); i++) {
            sb.setCharAt(i, removeAccentNonSpace(sb.charAt(i)));
        }
        return sb.toString();
    }
}